...
/**
 * Masking is on globally, and a per-URL override turns it off for the echo endpoint.
 * The registered handler still masks {@code phone} everywhere else.
 */
@TestPropertySource(properties = {
        "wiretap.rest-controllers.enable-request-params-masking=true",
        "wiretap.rest-controllers.specific-http-info-settings[0].match-url-pattern=.*/api/echo.*",
        "wiretap.rest-controllers.specific-http-info-settings[0].enable-request-params-masking=false"
})
class HttpMaskingPerUrlDisabledTest extends WiretapIntegrationTestBase {

    @Autowired
    TestRestTemplate restTemplate;

    @Test
    void perUrlOverrideKeepsQueryParamsUnmasked(CapturedOutput output) {
        restTemplate.getForEntity(
            "/api/echo?marker=per-url-off&phone=79991234567",
            Map.class
        );
        ...
        Map<String, Object> log = JsonLogCapture.awaitMatching(output, e -> {
            ...
            return params != null && "per-url-off".equals(asFirst(params.get("marker")));
        });

        Map<String, Object> params = JsonLogCapture.at(log, "http_info.request_params");
        assertThat(asFirst(params.get("phone"))).isEqualTo("79991234567");
    }
