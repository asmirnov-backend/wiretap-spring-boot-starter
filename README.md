...
**Status:** `2.1.0` is the current release — available on Maven Central. The public API (configuration properties, SPI interfaces, artifact coordinates) follows [semantic versioning](https://semver.org/spec/v2.0.0.html) from 1.0.0 on. Coming from `2.0.x` nothing needs changing; coming from `1.x`, the 2.0.0 major bump made settings fields nullable and changed a few accessor signatures — YAML configuration was unaffected, so see the [CHANGELOG](CHANGELOG.md) only if you implement `BodyParser` or read the settings classes from code. Please keep reporting what breaks on real projects (that's what open source is for).
